document.addEventListener("DOMContentLoaded", function() {
    ...
    const command = '/getIntervalWashedCarpets';
    ...
    document.getElementById("showDataButton").addEventListener("click", () => {
        ...
        .then(data => {
            ...
            data.date.forEach(item => {
                ...
                row.insertCell(0).textContent = item.washDay;
                ...
                row.addEventListener('dblclick', () => {
                    const washDay = item.washDay; // Получаем значение washDay из текущего элемента
                    const id = item.id || ''; 
                    window.location.href = `./concretDayWashedCarpets/concretDayWashedCarpets.html?ամսաթիվ=${encodeURIComponent(washDay)}`;
                });
            });
